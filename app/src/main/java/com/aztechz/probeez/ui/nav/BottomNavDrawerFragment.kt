...
import com.aztechz.probeez.util.themeInterpolator
...
class BottomNavDrawerFragment :
    ...
    override fun onNavTaskFolderClicked(folder: NavigationModelItem.NavTaskFolder) {
        navigationListeners.forEach { it.onNavTaskFolderClicked(folder) }
    }
    ...
    private fun toggleSandwich() {
        ...
        sandwichAnim = ValueAnimator.ofFloat(initialProgress, newProgress).apply {
            ...
            duration = (abs(newProgress - initialProgress) *
                resources.getInteger(R.integer.probeez_motion_duration_medium)).toLong()
        }
        sandwichAnim?.start()
    }
